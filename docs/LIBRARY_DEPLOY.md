...
3. Откройте **Библиотека** в шапке и переключите режим на **Онлайн**.
4. Если сервер новый (пустая БД), появится окно **Создание администратора** — задайте логин и пароль первого admin.
5. Для повторного входа: **Настройки → Библиотека** — логин и пароль → **Сохранить пароль**, затем **Проверить подключение к библиотеке**.
6. Управление пользователями и подписками: **Библиотека → Онлайн** → вкладка **Управление пользователями** (для admin).
...
- Задайте надёжный `AUTH_TOKEN_SECRET` на публичном VPS.
- Первый admin создаётся только при пустой таблице `users` (через UI или `POST /api/auth/setup`).
